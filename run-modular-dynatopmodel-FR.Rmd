...
read.spatial <- readRDS('SpatialInputData/dynatop_spatial_FR.RData')
...
read.hydromet <- readRDS('HydroInputData/dynatop_hydromet_FR_short.RData')
...
model.drty <- 'D:/Data/Dynamic-TOPMODEL/Falling-Rock/Calibration-2'
...
saveRDS(out,'D:/Data/Dynamic-TOPMODEL/Falling-Rock/Calibration-2/dynatop_calib_FR.RData')
```
...
read.out.calib <- readRDS('D:/Data/Dynamic-TOPMODEL/Falling-Rock/Calibration-2/dynatop_calib_FR.RData')

optim.params <- as.numeric(model.params[1,-length(model.params)])
...
out.routing <- explicit.routing.instant(read.spatial=read.spatial,explicit.reach.table=explicit.reach.table,run=result.optim$run,model.timestep=model.timestep)
...
qin.files.dir <- 'D:/Data/Dynamic-TOPMODEL/Falling-Rock/Calibration-2/fluxes_stores/'
...
kge.out.behavioral <- data.frame(matrix(nrow=length(qin.files),ncol=1))

for (iter.postprocess in 1:length(qin.files)) {
  kge.out.behavioral[iter.postprocess,1] <- headwater.postprocess.behavioral[[iter.postprocess]]$kge$x
  
}

percent.correct.state.FC$kge <- kge.out.behavioral[,1]
...
network.length.total.fig <- (headwater.postprocess.behavioral[[18]]$total.network.on.off[1:nrow(network.length.postprocess),])
...
kge.totalpercentcorrect.flowthresh <- data.frame(kge=percent.correct.state.FC$kge,total.percent.correct=total.percent.correct.FC$percent.correct,flow.thresh=optimal.flow.thresh.mm.day)
...
logger.test <- data.frame(state=TS.zoo.all[[2]],
    ...
    # TYLER YOU WILL NEED TO CHANGE THE REACH NAMES BELOW DON'T FORGET 8/23/2023
    
kge.in <- data.frame(kge=percent.correct.state.FC$kge)
    # Create a data frame for the flow threshold, FC1 percent correct, and the kge and plot
kge.FC1percentcorrect.flowthresh <- data.frame(kge=kge.in,FC1.percent.correct=percent.correct.state.FC$`3722`,flow.thresh=(optimal.flow.thresh.mm.day))
...
kge.FC2percentcorrect.flowthresh <- data.frame(kge=kge.in,FC2.percent.correct=percent.correct.state.FC$`3242`,flow.thresh=(optimal.flow.thresh.mm.day))
...
# Create a data frame for the flow threshold, FC2 percent correct, and the kge and plot
kge.FC3percentcorrect.flowthresh <- data.frame(kge=kge.in,FC3.percent.correct=percent.correct.state.FC$`2538`,flow.thresh=(optimal.flow.thresh.mm.day))
...
kge.FC4percentcorrect.flowthresh <- data.frame(kge=kge.in,FC4.percent.correct=percent.correct.state.FC$`810`,flow.thresh=(optimal.flow.thresh.mm.day))
...
plot.fc1 <- ggplot(data=kge.FC1percentcorrect.flowthresh,aes(x=kge,y=FC1.percent.correct,color=flow.thresh))+ geom_point() + xlab('KGE') + ylab('Percent Flow State Correct (%)') +scale_colour_viridis_c() + labs(color='Transmissivity (m^3/hr)') + ggtitle('(a) FC1') + ylim(60,100)+theme_bw()+theme(legend.position='none')+xlim(0.3,0.8)

plot.fc2 <- ggplot(data=kge.FC2percentcorrect.flowthresh,aes(x=kge,y=FC2.percent.correct,color=flow.thresh))+ geom_point() + xlab('KGE') + ylab('Percent Flow State Correct (%)') +scale_colour_viridis_c() + labs(color='Transmissivity (m^3/hr)') + ggtitle('(b) FC2') + ylim(60,100)+theme_bw()+theme(legend.position='none')+xlim(0.3,0.8)

plot.fc3 <- ggplot(data=kge.FC3percentcorrect.flowthresh,aes(x=kge,y=FC3.percent.correct,color=flow.thresh))+ geom_point() + xlab('KGE') + ylab('Percent Flow State Correct (%)') +scale_colour_viridis_c() + labs(color='Transmissivity (m^3/hr)') + ggtitle('(C) FC3') + ylim(60,100)+theme_bw()+theme(legend.position='none')+xlim(0.3,0.8)

plot.fc4 <- ggplot(data=kge.FC4percentcorrect.flowthresh,aes(x=kge,y=FC4.percent.correct,color=flow.thresh))+ geom_point() + xlab('KGE') + ylab('Percent Flow State Correct (%)') +scale_colour_viridis_c() + labs(color='Transmissivity (m^3/hr)') + ggtitle('(d) FC4') + ylim(60,100)+theme_bw()+theme(legend.position='none')+xlim(0.3,0.8)
...
on.vs.area <- data.frame(reach = read.spatial$explicit.ChanTable$link_no,upstream.area = read.spatial$explicit.ChanTable$US_area_m2,percent.on.avg = network.percent.on.stats$mean,
           ...
           best.percent.on=network.percent.on.df$X180)
