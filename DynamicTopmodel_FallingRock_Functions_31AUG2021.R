...
Headwater.evaluation <- function(qin.file, FR.Spatial, TS.logger.clean.all, TS.zoo.all, logger.out.percent, Reach.identifiers, Reach.identifiers.second,model.timestep...) {
  ...
  # Run routing function -- note we want reach-specific flow in runoff (m/hr)
  ...
  ## Run the analysis for 10000 flow thresholds, output the optimum flow threshold. 
  min.threshold <- 6.68331e-08                                                     # This was the minimum optimal value from the 10-fold cross validation investigation of thresholds
  max.threshold <- 2.861665e-03                                                    # This was the maximum optimal value from the 10-fold cross validation
  flow.thresh.range <- seq(from=log(min.threshold), to = log(max.threshold),                 # Creates a sequency of 1000 thresholds for which we'll calculate the % correct
                           by = (log(max.threshold)-log(min.threshold))/999)                 # Continued 
  ...
  for (name.logger in 1:length(names.loggers)) {
    ...
    sim.obs.difference[[name.logger]] <- data.frame(
      matrix(nrow=length(logger.timeseries[[name.logger]]$Date),ncol=length(flow.thresh.range)+1))
    
    for (thresh.in in 1:length(flow.thresh.range)) {                                              # Run the analysis
      # Write the simulated 1/0 time series for the average threshold
      
      thresh <- exp(flow.thresh.range[thresh.in])
      ...
      explicit.r.names <- names(explicit.routing.run$routed.flow.instant.mm_s)
      sim.810.on <- 
        ifelse(explicit.routing.run$routed.flow.instant.mm_s[which(explicit.r.names==Reach.identifiers.second[name.logger])]>thresh,           # USing the iterative threshold
      ...
      if (thresh.in==1) {
        out.thresh.df[[name.logger]] <- data.frame(percent.correct=correct.state.percent.thresh,
                                                   flow.thresh=thresh,percent.thresh=percent.thresh, sum.correct=sum(true.on,true.off),
      ...
      if (thresh.in==1) {
        ...
        sim.obs.difference[[name.logger]][,thresh.in+1] <- sim.obs.match$difference
      } else {
        sim.obs.difference[[name.logger]][,thresh.in+1] <- sim.obs.match$difference
      }
    }
  ...
  residual.on.off <- matrix(nrow=length(flow.thresh.range),ncol=length(names.loggers))
  ...
  rowSums.residual.on.off <- data.frame(sum.error=rowSums(residual.on.off),thresh=(flow.thresh.range))
  
  # calculate which threshold produces the minimum error accross all four sensors
  thresh.best <- exp(rowSums.residual.on.off$thresh[which.min(rowSums.residual.on.off$sum.error)])
  
  # Output results for all thresholds into a list
  flow.thresh.best.results <- 
    list(sim.obs.difference=sim.obs.difference, out.thresh.data=out.thresh.df,
         confusion.matrix=confusion.matrix.thresh)
  ...
  total.network.on.off <- data.frame(ifelse(explicit.routing.run$routed.flow.instant.mm_s>(thresh.best),1,0))
  flow.network.thresh <- explicit.routing.run$routed.flow.instant.mm_s
  colnames(flow.network.thresh) <- r.names
  flow.network.thresh[flow.network.thresh < (thresh.best)] <- 0
  ...
  order.1.on <- 
    ifelse(explicit.routing.run$routed.flow.instant.mm_s$`810`>thresh.best, # USing the averaged threshold
           1,0)                                                                    # Set the values greater than the threshold to one, otherwise set to zero 
  ...
  out.headwater.evaluation <- list(                                       # Compile a list of everything we want to output
    stats.on.off=stats.on.off,flow.thresh.best.results=flow.thresh.best.results,
