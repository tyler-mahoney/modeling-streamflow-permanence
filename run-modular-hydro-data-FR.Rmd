...
```{r}
try(Qobs.1971.1999 <- read.csv('HydroInputData/FR_Qobs1971_1999.csv'))
try(Qobs.2000.2018 <- read.csv('HydroInputData/FR_Qobs2000_2015.csv'))

if(is.null(Qobs.1971.1999) | is.null(Qobs.2000.2018)) {

raw.Qin <- read.csv('HydroInputData/FRStreamflowCompiled.csv')
...
write.csv(Qobs.1971.1999,'HydroInputData/FR_Qobs1971_1999.csv')
write.csv(Qobs.2000.2018,'HydroInputData/FR_Qobs2000_2015.csv')
}
...
Q.data.in <- read.csv('HydroInputData/FR_Qobs2000_2015.csv')
...
```{r}
try(logger.clean.1 <- read.csv('HydroInputData/FR1_short_cleanFR.csv'))
...
if(is.null(logger.clean.1)) {
  ...
  for (i in files.logger) {
    ...
    write.csv(logger.clean.final,paste0('HydroInputData/',name,'_cleanFR.csv'))
  }
...
files.logger <- list.files(logger.dir,pattern='*_short_cleanFR.csv')                        # List files in the above directory
reach.loggers <- read.csv('HydroInputData/FR-reaches-loggers-short.csv')
TS.zoo.all <- list()
...
for (file.name in 1:length(files.logger)) {                                                  # Iterate over each file
  ...
  short.name <- substr(files.logger[file.name],start=1,
                       stop=(nchar(files.logger[file.name])-18))    # 10 if the longer dataeset is used           # Create name for the iteration
...
saveRDS(dynatop.hydro.met,'HydroInputData/dynatop_hydromet_FR_short.RData')
